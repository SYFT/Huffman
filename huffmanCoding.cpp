...
int main(int argc, char *argv[]) {
    ...
    int start = time(0);
    if(argc == 3) {
        HuffmanCode::HCState S = HuffmanCode::compress(argv[1], argv[2]);
        if(S == HuffmanCode::wrong_file)
            ...
            cout << "Filesize after compressing is " << fixed <<
                setprecision(2) << sizeB / 1024 << "KB" << endl;
            cout << "Compression ratio is " << fixed << setprecision(2) <<
                (sizeA - sizeB) / sizeA * 100. << "%" << endl;
            int end = time(0);
            cout << "Cost time " <<
